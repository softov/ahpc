...
**Read a capture, not only the declarations.** The protocol says a turn's model is `Message.model`. On a captured conversation it is absent on every turn, and the model is in two places instead: `usage.model`, which is declared and is a plain string, and `SessionState.model`, which **no version of the protocol declares at all**. This client read the declared field, and read it with a string test against an object, so it showed no model against any host for its whole life - and every test agreed with it, because the fixtures were written from the same declarations as the code. A type says what a host *may* send. Only a capture says what one *does*.
...
## B-01-20 - A thinking level is chosen through the session, not the turn

`ModelSelection` is carried inbound: a turn keeps the settings it ran with, the transcript says them beside the model, and the export records both. The question was what to do outbound, and it has an answer now rather than a preference.

**What the reference client does, from a capture of it.** It never sends `ModelSelection.config` at all. The effort level is a *session config* key: it appears in `resolveSessionConfig`'s `config` while the form is being resolved, and in `createSession`'s `config` alongside `permissionMode` and `isolation`. Not one of its seven dispatched actions carries a model, and no turn in that capture names one. So the control that exists in the other implementation is the session config control, and it is the one this client already has.

**Which contradicts the schema, and the schema loses.** `state.schema.json` says of `configSchema` that clients present it as a form and pass the resolved values in `ModelSelection.config`. ahpd honours that path as of `5f0b040`. No shipped client uses it. Building a per-turn control on it would mean this client alone exercising a route the other implementation does not, against the only other host that does not read it - which is how a client ends up correct and alone.

**So the outbound half is not a control, it is a narrowing.** `setConfig` already dispatches `session/configChanged`, and `createSession` already carries `config`, so a person can already set the effort level here. What is missing is that the session-wide choice is offered whole when the model running under it accepts only some of those values - and ahpd has taken the other side of the same problem, offering `effortLevel` only when no model carries a schema of its own.

**Suggestions.** (1) Narrow the session config control to the model's `options` where both name the same thing, leaving the host's key and the host's words in charge - the model constrains the choice rather than adding a second one. (2) Nothing, and let a host that offers both draw both, which is what produced two effort pickers on different values in the reference window until ahpd stopped sending one. (3) Send `ModelSelection.config` as well as the session key, which is the specification's route and doubles the ways one setting can be expressed.
