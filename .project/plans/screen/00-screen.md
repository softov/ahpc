...
The other front end, the shell in `src/cli/`, is its own domain; `src/config.ts`, `src/flags.ts` and `src/version.ts` are shared by both.
...
- `code://src/state.ts` - the store keys (`HOST_ERROR`, `INPUT_STATUS`, `SCREEN`, ...) that the surfaces and screens bind to.
...
ahpc [flags] -> main.tsx: --version answered, else no command word -> tui(argv)
  ...
  -> ChatStatus draws HOST_ERROR when set, else the key hints, plus the screen name
```
