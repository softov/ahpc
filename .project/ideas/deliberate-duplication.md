...
The update check is the second copy: `src/update.ts` here and `packages/server/src/update.ts` in `ahpd`, the same five functions, with the same comparison table in each repository's `test/update.test.ts` in the same order.
